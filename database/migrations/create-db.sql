...
CREATE TABLE Sync_CursoSeccionBasica (
    cursoId INT NULL
    , cCurso VARCHAR(255) NOT NULL
    , nSeccionId INT NOT NULL
    , cSeccion VARCHAR(255) NOT NULL
    , cGrado VARCHAR(255) NOT NULL
    , cNivel VARCHAR(255) NOT NULL
    , idPeriodoLectivo INT NOT NULL
    , CONSTRAINT UK_Sync_CursoSeccionBasica UNIQUE (cursoId)
    );
...
CREATE TABLE Sync_CursoSeccionPreUniversitaria (
    cursoId INT NULL
    , cCurso VARCHAR(255) NOT NULL
    , cAulaId INT NOT NULL
    , cAula VARCHAR(255) NOT NULL
    , cCentroDeEstudios VARCHAR(255) NOT NULL
    , cAreaAcademica VARCHAR(255) NOT NULL
    , cAreaCorricular VARCHAR(255) NOT NULL
    , idPeriodoLectivo INT NOT NULL
    , idTemporada INT NULL
    , CONSTRAINT UK_Sync_CursoSeccionPreUniversitaria UNIQUE (cursoId)
    );
...
CREATE TABLE Vigencia (
    fechaLimiteId_pk INT NOT NULL
    , horarioDiasId_pk INT NOT NULL
    , bEliminado BIT DEFAULT 0 NOT NULL
    , nCreatedBy INT NOT NULL
    , tCreatedAt DATE DEFAULT GETDATE() NOT NULL
    , CONSTRAINT PK_Vigencia PRIMARY KEY (
        fechaLimiteId_pk
        , horarioDiasId_pk
        )
    , CONSTRAINT FK_FechaLimite_Vigencia FOREIGN KEY (fechaLimiteId_pk) REFERENCES FechaLimite(id)
    , CONSTRAINT FK_HorarioDias_Vigencia FOREIGN KEY (horarioDiasId_pk) REFERENCES HorarioDias(id)
    );
...
CREATE TABLE CursoSeccionBasica_TurnoRegular (
    ...
    , CONSTRAINT FK_SyncCursoSeccionBasica_TurnoRegular FOREIGN KEY (syncCursoSeccionId) REFERENCES Sync_CursoSeccionBasica(cursoId)
...
CREATE TABLE CursoSeccionPreUniversitaria_TurnoRegular (
    ...
    , CONSTRAINT FK_SyncCursoSeccionPreUniversitaria_TurnoRegular FOREIGN KEY (syncCursoSeccionPreUniversitariaId) REFERENCES Sync_CursoSeccionPreUniversitaria(cursoId)
